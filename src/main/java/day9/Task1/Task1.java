 ...
 * Метод printInfo() в классе Студент должен быть переопределен таким образом, чтобы формат выводимого
 * в консоль сообщения был таким:
 *
 ...
 * дополнительное строковое поле - “название предмета” (англ. subjectName), который ведет этот преподаватель.
 * Для этого поля необходимо создать get метод. Конструктор в классе Преподаватель должен
 * принимать на вход два аргумента - имя преподавателя и название преподаваемого предмета.
